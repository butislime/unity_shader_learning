...
public class BoxFilterBlur : MonoBehaviour
{
	...
	private void Awake()
	{
		...
		//cmd.GetTemporaryRT(prop2, Screen.width / 2, Screen.height / 2, 0, FilterMode.Bilinear, RenderTextureFormat.ARGB32);

		cmd.Blit(BuiltinRenderTextureType.CurrentActive, source);
		// ダウンサンプリングパス(0番)でレンダリング
		cmd.Blit(source, prop1, blurMaterial, 0);
		//cmd.Blit(prop1, prop2, blurMaterial, 1);
		// アップサンプリングパス(1番)でレンダリング
		//cmd.Blit(prop2, prop1, blurMaterial, 2);
		cmd.Blit(prop1, BuiltinRenderTextureType.CameraTarget, blurMaterial, 2);

		cmd.ReleaseTemporaryRT(source);
		cmd.ReleaseTemporaryRT(prop1);
		//cmd.ReleaseTemporaryRT(prop2);

		targetCamera.AddCommandBuffer(CameraEvent.AfterImageEffects, cmd);
	}
}
